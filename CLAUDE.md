...
- **Workspace Persistence**: AgentCore Session Storage (primary, `/mnt/workspace`) + S3 cold backup (30 min). `.openclaw/` symlinked to session storage mount; S3 backup restores on new sessions or version updates
...
- **S3 cold backup**: `workspace-sync.js` runs in backup mode (every 30 min) when session storage is available. Backs up to `{namespace}/.openclaw/` in the user files S3 bucket
